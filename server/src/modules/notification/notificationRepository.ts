...
class NotificationRepository {
  async findByUserId(userId: number): Promise<Notification[]> {
    const [rows] = await databaseClient.query<NotificationRow[]>(
      `
        SELECT
          id,
          user_id,
    ...
    return rows.map((row) => ({
      ...
      reference_id:
        row.reference_id !== null ? Number(row.reference_id) : null,
  ...
  async countUnreadByUserId(userId: number): Promise<number> {
    const [rows] = await databaseClient.query<
      (RowDataPacket & { unread_count: number })[]
    >(
      `
  ...
  async create(payload: CreateNotificationPayload): Promise<number> {
